...
def join_channel(token, channel):
    url = "https://slack.com/api/conversations.join?token={}&channel={}&pretty=1(".format(
        token, channel
    )
    r = requests.post(url)
    all_response = r.json()
...
def get_last_message(token, channel):
    url = "https://slack.com/api/conversations.history?token={}&channel={}&limit=1&pretty=1".format(
        token, channel
    )
    r = requests.get(url)
    all_response = r.json()
